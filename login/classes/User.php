...
require_once($root.'/login/classes/twitchtv.php');
...
class User
{
    ...
    private $user_email = "";
     /**
     * @var boolean $user_is_admin The user is Admin
     */
    ...
    public function generate_blog_toggles(){
      if ($this->databaseConnection()) {
        $query_get_blogs = $this->db_connection->prepare('SELECT * FROM ');
      }
    }

    private function createNewPost($post_title, $post_description, $post_content){

            $post_author_id = $_SESSION['user_name'];
            ...
            elseif(empty($post_description)){
              $this->errors[] = "Post description cannnot be empty (for now).";
            }
            elseif(empty($post_content)){
              ...
              $query_add_post = $this->db_connection->prepare('INSERT INTO posts (post_title, post_description, post_content, post_date, post_author_id) VALUES(:post_title, :post_description, post_content, now(), post_author_id)');
              $query_add_post->bindValue(':post_title', $post_title, PDO::PARAM_STR);
              $query_add_post->bindValue(':post_description', $post_description, PDO::PARAM_STR);
              $query_add_post->bindValue(':post_content', $post_content, PDO::PARAM_STR);
              $query_add_post->bindValue(':post_author_id', $post_author_id, PDO::PARAM_STR);
              $query_add_post->execute();

              $post_id = $this->db_connection->lastInsertId();

              $email_id = $this->db_connection->lastInsertId();
    ...
    private function registerNewEmail($user_real_name, $user_email){
      $user_email = trim($user_email);
      $user_real_name = trim($user_real_name);
      if ($this->databaseConnection()) {

        $query_check_user_email = $this->db_connection->prepare('SELECT user_email FROM users WHERE user_email=:user_email');
        ...
        if (count($result) > 0){
          ...
          $query_new_user_insert = $this->db_connection->prepare('INSERT INTO users (user_real_name, user_email, user_activation_hash, user_registration_datetime) VALUES(:user_real_name, :user_email, :user_activation_hash, now())');
      ...
      if ($query_new_user_insert) {
          ...
          if ($this->registration_successful){
            ...
            $esc = $_POST['blog_3'];
            if ($cgc){
              $query_email_subscription = $this->db_connection->prepare('INSERT INTO email_subscription (blog_id, user_email) VALUES(1, :user_email)');
              $query_email_subscription->bindValue(':user_email', $user_email, PDO::PARAM_STR);
              $query_email_subscription->execute();
              $this->sendEmail("no-reply@gamu.ca", "No Reply GAMU", $user_email, "Welcome", "Welcome to the Casual Games Club! Your email has been stored in our database so the the club can contact you. They will contact you further once the complete list is compiled.");
            }
            if ($vgc){
              $query_email_subscription = $this->db_connection->prepare('INSERT INTO email_subscription (blog_id, user_email) VALUES(2, :user_email)');
              $query_email_subscription->bindValue(':user_email', $user_email, PDO::PARAM_STR);
              $query_email_subscription->execute();
              $this->sendEmail("no-reply@gamu.ca", "No Reply GAMU", $user_email, "Welcome", "Welcome to the Video Games Club! Your email has been stored in our database so the the club can contact you. They will contact you further once the complete list is compiled.");
            }
            if ($esc){
              $query_email_subscription = $this->db_connection->prepare('INSERT INTO email_subscription (blog_id, user_email) VALUES(3, :user_email)');
              $query_email_subscription->bindValue(':user_email', $user_email, PDO::PARAM_STR);
              $query_email_subscription->execute();
              $this->sendEmail("no-reply@gamu.ca", "No Reply GAMU", $user_email, "Welcome", "Welcome to the Esports Club! Your email has been stored in our database so the the club can contact you. They will contact you further once the complete list is compiled.");
            }
          }
      } else {
    ...
    private function registerNewUser($user_name, $user_real_name, $user_email, $user_password, $user_password_repeat)
    {
        ...
        if (empty($user_real_name)) {
            $this->errors[] = MESSAGE_USER_REAL_NAME_EMPTY;
        } elseif(empty($user_name)){
          if (! empty($user_email)){
              $this->registerNewEmail($user_real_name, $user_email);
          }
          else{
    ...
    public function sendVerificationEmail($user_id, $user_email, $user_activation_hash)
    {
        ...
        $mail->Body = EMAIL_VERIFICATION_CONTENT.' '.$link . "\n\nAt this moment the website is still under construction so most features are still not complete or fully implemented. Because of that an email will be sent when the website is complete so that your preferences can be more easily controlled. Sorry for any inconveniences this may cause";
    ...
    private function getUserDataByName($user_real_name)
    {
        // if database connection opened
        if ($this->databaseConnection()) {
            // database query, getting all the info of the selected user
            $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE user_real_name = :user_real_name');
            $query_user->bindValue(':user_real_name', $user_real_name, PDO::PARAM_STR);
            $query_user->execute();
            // get result row (as an object)
            return $query_user->fetchObject();
        } else {
    ...
    private function loginWithPostData($user_login, $user_password, $user_rememberme)
    {
        ...
        // if POST data (from login form) contains non-empty user_name and non-empty user_password
            ...
            // if user has typed a valid email address, we try to identify him with his user_email
                ...
                // database query, getting all the info of the selected user
                $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE user_name = :user_login');
                $query_user->bindValue(':user_login', trim($user_login), PDO::PARAM_STR);
                $query_user->execute();
                // get result row (as an object)
            ...
            // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
                ...
                $sth = $this->db_connection->prepare('UPDATE users '
                        ...
                        . 'WHERE user_name = :user_login OR user_email = :user_login OR user_real_name = :user_login');
                $sth->execute(array(':user_login' => $user_login, ':user_last_failed_login' => time()));
